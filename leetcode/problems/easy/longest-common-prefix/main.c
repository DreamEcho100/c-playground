...
char *longestCommonPrefix(char **strs, int strsSize)
{
	...
	// Ensures that memory is initialized to zero using `calloc`, making the string null-terminated from the start
	...
	for (int i = 0; i < minLength; i++)
	{
		...
		{
			if (
					// The following is commented because it's not needed since we fond the min length earlier
					// currentChar == NULL_TERMINATOR_CHAR ||
